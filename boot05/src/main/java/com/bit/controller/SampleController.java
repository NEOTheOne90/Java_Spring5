...
public class SampleController {
	...
	@GetMapping("/sample1") //sample1.html 호출
	...
	public void sample3(Model model) {
		...
		for (int i = 0; i < 10; i++) {
			list.add(new MemberVO(123, "u00", "p00", "홍길동", 
						new Timestamp(System.currentTimeMillis())));
		}
	...
	public void sample6(Model model) {
		List<MemberVO> list = new ArrayList<MemberVO>();
		for (int i = 0; i < 10; i++) {
			list.add(new MemberVO(123, "u0"+i%3, "p0"+i%3, "홍길동"+i, 
						new Timestamp(System.currentTimeMillis())));
		}
		model.addAttribute("list", list);
		model.addAttribute("result", "SUCCESS");
	...
	public void sample7(Model model) {
		...
		model.addAttribute("title", "This is a just sanple.");
